...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!series) {
            ...
            const seriesInfo = await DataServiceNew.getDatasetsByAssetClass('bonds');
        ...
        const data = await DataServiceNew.loadCSV(`bonds/${series}`, columns);
        ...
        return NextResponse.json(
            { error: 'Failed to load bonds data' },
            { status: 500 }
        );
    }
}