...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!series) {
            ...
            const seriesInfo = await DataServiceNew.getDatasetsByAssetClass('volatility');
        ...
        const data = await DataServiceNew.loadCSV(`volatility/${series}`, columns);
        ...
        return NextResponse.json(
            { error: 'Failed to load volatility data' },
            { status: 500 }
        );
    }
}