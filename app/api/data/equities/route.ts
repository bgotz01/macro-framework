...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!series) {
            ...
            const seriesInfo = await DataServiceNew.getDatasetsByAssetClass('equities');
        ...
        const data = await DataServiceNew.loadCSV(`equities/${series}`, columns);
        ...
        console.error('Equities API Error:', error);
        return NextResponse.json(
            { error: 'Failed to load equities data' },
            { status: 500 }
        );
    }
}