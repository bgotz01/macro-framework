...
function createSnapshots() {
    console.log('\n📸 Creating S&P 500 Historical Snapshots (v2)\n');
    ...
    // Now use Wharton data to work backwards from 2022-12-31
    console.log('\nUsing Wharton data for 2000-2021...\n');

    // Get the 2022-12-31 baseline
    const baseline2022 = db.prepare(`
        SELECT ticker, company_name
        FROM sp500_snapshots
        WHERE snapshot_date = '2022-12-31'
    `).all() as Array<{ ticker: string; company_name: string }>;

    console.log(`Starting from 2022-12-31 baseline: ${baseline2022.length} companies`);

    // Get all Wharton changes sorted by date descending
    ...
    console.log(`Wharton changes to process: ${whartonChanges.length}\n`);

    // Work backwards year by year
    let composition = new Map<string, string>();
    baseline2022.forEach(({ ticker, company_name }) => {
        composition.set(ticker, company_name || ticker);
    });

    for (let year = 2021; year >= 2000; year--) {
        const snapshotDate = `${year}-12-31`;
        const targetDate = new Date(snapshotDate);
        const nextDate = new Date(`${year + 1}-01-01`);

        // Apply all changes that happened between target date and next year
        for (const change of whartonChanges) {
            const changeDate = new Date(change.date);

            // Only process changes that happened after our target date
            // and on or before the end of 2022
            if (changeDate <= targetDate) continue;
            if (changeDate >= new Date('2023-01-01')) continue;

            // Reverse the change
            if (change.action === 'added') {
                // Company was added after target date, so remove it
                composition.delete(change.ticker);
            } else if (change.action === 'removed') {
                // Company was removed after target date, so add it back
                composition.set(change.ticker, change.ticker);
            }
        }
        ...
        const entries: Array<[string, string, string]> = Array.from(composition.entries())
            .map(([ticker, name]) => [snapshotDate, ticker, name]);

        insertMany(entries);

        console.log(`✓ ${snapshotDate}: ${composition.size} companies`);
    }
