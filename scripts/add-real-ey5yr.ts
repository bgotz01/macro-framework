...
function addRealEY5yr() {
    const dbPath = path.join(process.cwd(), 'data', 'macro-data.db');
    const db = new Database(dbPath, { timeout: 30000 });
    db.pragma('journal_mode = WAL');

    try {
        console.log('📊 Creating Real Earnings Yield 5yr (EY-5yr - CPI)...\n');

        // First, create the combined data
        console.log('Step 1: Combining EY-5yr and CPI data...');
        const combinedQuery = `
            SELECT 
                pa1.date,
        ...
        for (let i = 0; i < combinedData.length; i++) {
            ...
            const rankBelow = historicalData.filter(d => d.rey5yr < current.rey5yr).length;
        ...
        console.log('\n✅ Real Earnings Yield 5yr percentiles created successfully!');
